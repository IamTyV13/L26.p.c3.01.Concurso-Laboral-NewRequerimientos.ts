...
export default class Cl_vAspirante implements I_vAspirante {
    ...
    // Arrays para que lleguen los puntos de cada jurado a la clase menor
        ptsJuradoA(): number[] {
            const puntos: number[] = [];
            
            for (let b = 1; b <= 12; b++) {
                const input = document.getElementById(`aspirante_inJuradoA${b}`) as HTMLInputElement;
                ...
                if (input) {
                    ...
                    // Validar que no sea negativo
                    if (valor < 0) {
                        alert(` Puntuacion ${b} no valida, los puntos no puede ser negativos`);
                            return [];  // Retorna array vacío si hay error
                    }
        ...
        ptsJuradoB(): number[] {
            ...
            for (let b = 1; b <= 12; b++) {
                const input = document.getElementById(`aspirante_inJuradoB${b}`) as HTMLInputElement;
        ...
        ptsJuradoC(): number[] {
            ...
            for (let b = 1; b <= 12; b++) {
                const input = document.getElementById(`aspirante_inJuradoC${b}`) as HTMLInputElement;
    ...
    // Metodo para Validar el valor máximo y mínimo que se puede agregar a la pregunta
        private aplicarValidaciones(): void {
            const validarExamen = (input: HTMLInputElement) => {
                if (!input) return;
            ...
            validarExamen(document.getElementById("aspirante_inNotaExamenEscrito") as HTMLInputElement);
            validarExamen(document.getElementById("aspirante_inNotaExamenPractico") as HTMLInputElement);
            validarExamen(document.getElementById("aspirante_inNotaExamenAptitudes") as HTMLInputElement);
